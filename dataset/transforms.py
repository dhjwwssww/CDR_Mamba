...
class Resize_3d:
    def __init__(self, scale0, scale1, scale2):
        # self.shape = [shape, shape, shape] if isinstance(shape, int) else shape
        self.scale0 = scale0
        self.scale1 = scale1
        self.scale2 = scale2
    ...
    def __call__(self, img, mask):
        img, mask = img.unsqueeze(0), mask.unsqueeze(0).float()
        img = F.interpolate(img, scale_factor=(self.scale0, self.scale1, self.scale2),mode='trilinear', align_corners=False, recompute_scale_factor=True)
        mask = F.interpolate(mask, scale_factor=(self.scale0, self.scale1, self.scale2), mode="nearest", recompute_scale_factor=True)
        return img[0], mask[0]

class Resize_abs:
    def __init__(self,s_rank, h_rank, w_rank):
        self.w_rank = w_rank
        self.h_rank = h_rank
        self.s_rank = s_rank
...
class Crop:
    def __init__(self, range1, range2, range3):
        self.start1, self.end1 = range1
        self.start2, self.end2 = range2
        self.start3, self.end3 = range3
    ...
    def __call__(self, img, mask):
        ...
        # print(self.shape, img.shape, mask.shape)
        tmp_img = torch.zeros((img.size(0), self.end1 - self.start1, self.end2 - self.start2, self.end3 - self.start3))
        tmp_mask = torch.zeros((mask.size(0),  self.end1 - self.start1, self.end2 - self.start2, self.end3 - self.start3))
        tmp_img[:, :self.end1 - self.start1, :self.end2 - self.start2, :self.end3 - self.start3] = img[:, self.start1:self.end1, self.start2:self.end2, self.start3:self.end3]
        tmp_mask[:, :self.end1 - self.start1, :self.end2 - self.start2, :self.end3 - self.start3] = mask[:, self.start1:self.end1, self.start2:self.end2, self.start3:self.end3]
        return tmp_img, tmp_mask
...
class Flip_LR:
    ...
    def __call__(self, img, mask):
        return self._flip(img), self._flip(mask)
